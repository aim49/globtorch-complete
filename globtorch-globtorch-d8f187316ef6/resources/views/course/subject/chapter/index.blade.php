    ...
    <div class="row page-titles">
        <div class="col-md-3 align-self-center">
            <h3>
                ...
                @else
                    <a href="{{route('course.subject.index', $course->id)}}">{{$course->name}}/</a>
                @endif
                <a href="{{route('course.subject.chapter.index', [$course->id, $subject->id])}}">{{$subject->name}}/</a>
            </h3>
        </div>
        <div class="col-md-3 align-self-center">
                                        ...
                                        <div class="panel-body">
                                            <p>
                                                @if (count($chapter->topics) > 0)
                                                    @foreach ($chapter->topics as $topic)
                                                        <div class="card p-30">
                                                            ...
                                                            <br> 
                                                            <a href="{{route('course.subject.chapter.topic.show', [$course->id, $subject->id, $chapter->id, $topic->id])}}" class="btn btn-success video-button mt30 inline-block">View Content</a>
                                                            @if(auth()->user()->user_type!='student')
                                                                <a href="{{route('course.subject.chapter.topic.content.create', [$course->id, $subject->id, $chapter->id, $topic->id])}}" class="btn btn-primary video-button mt30 inline-block">Add Content</a>                                                    
                                                            @endif
                                                ...
                                                @if(auth()->user()->user_type !='student')
                                                    ...
                                                    <a href="{{route('course.subject.chapter.topic.create', [$course->id, $subject->id, $chapter->id])}}" class="btn btn-primary video-button mt30 inline-block">Add Topic</a>
        ...
        @if(auth()->user()->user_type !='student')
            <div class="card">
                <div class="card-body" style="text-align:center">                
                    <a href="{{route('course.subject.chapter.create', [$course->id, $subject->id])}}" class="btn btn-primary video-button mt30 inline-block">Add Chapter</a>               
                </div>
            </div>
        @endif
    </div>
